...
export const {
  reducer: volunteerDetailedKnowledgeListReducer,
  actions: volunteerDetailedKnowledgeListActions,
...
const selectShouldFetchVolunteerDetailedKnowledgeList = (state: AppState): boolean => state.volunteerDetailedKnowledgeList?.readyStatus !== 'success'

export const fetchVolunteerDetailedKnowledgeListIfNeed: AppThunk = (id = 0) => (dispatch: AppDispatch, getState: () => AppState) => {
  let jwt = ''
...
export function selectVolunteerDetailedKnowledgeListState(state: AppState): EntitiesRequest<VolunteerDetailedKnowledge> {
