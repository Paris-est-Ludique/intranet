...
const expressAccessor = new ExpressAccessors<GameWithoutId, Game>('Games', new Game(), translationGame)
...
export const gameDetailsUpdate = expressAccessor.listSet(async (list, _body, _id, _roles, request) => {
...
export const gameWithVolunteersListGet = expressAccessor.get(async (list, _body, id): Promise<GameWithVolunteers[]> => {
  if (id <= 0) {
  ...
  const giftGameIds = list
    .filter(game => every(volunteerList, v => !v.loanable.includes(game.id) && !v.playable.includes(game.id)))
    .map(game => game.id)
  ...
  const gamesToGift = list
    .filter(game => giftGameIds.includes(game.id) && currentVolunteer.loanable.includes(game.id))
    .map(game => ({
      ...cloneDeep(game),
      volunteerNicknames: volunteerToNicknameList(
        volunteerList.filter(v => v.loanable.includes(game.id)),
...
function volunteerToNicknameList(volunteers: Volunteer[], allVolunteers: Volunteer[]): string[] {
  return volunteers.map(v => getUniqueNickname(allVolunteers, v)).sort((a, b) => a.localeCompare(b))
}
...
export const gameTitleOrderCategories = expressAccessor.get(async (list): Promise<GameCategory[]> => {
  const volunteerSheet = await getSheet<VolunteerWithoutId, Volunteer>(
    'Volunteers',
    new Volunteer(),
    translationVolunteer,
  ...
  const exaustiveCats = mapValues(groupBy(gamesToLoan, gameTitleCategory), gameList => gameList.map(game => game.title))
