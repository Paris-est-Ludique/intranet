...
import type { ElementWithId, Sheet, SheetNames } from './accessors'
import { getSheet } from './accessors'

export type RequestBody = Request['body']
export interface CustomSetReturn<Element> {
  toDatabase: Element
  toCaller: any
}
export interface CustomAddReturn<Element> {
  toDatabase: Omit<Element, 'id'>
  toCaller: any
}

export default class ExpressAccessors<ElementNoId extends object, Element extends ElementWithId<ElementNoId>> {
  ...
  constructor(
    ...
    getSheet<ElementNoId, Element>(this.sheetName, this.specimen, this.translation).then(sheet => {
      this.sheet = sheet
      this.isLoaded = true
  ...
  parseRawPartialElement(rawPartialElement: Partial<Record<keyof Element, string>>): Partial<Element> | undefined {
    ...
    return async (request: Request, response: Response, _next: NextFunction): Promise<void> => {
      try {
        ...
        if (!custom) {
          ...
          const { toDatabase, toCaller } = await custom(list, request.body, memberId, roles, request, response)
  ...
  get<Ret = Element>(
    custom?: (list: Element[], body: Request['body'], id: number, roles: string[]) => Promise<Ret> | Ret,
  ) {
    return async (request: Request, response: Response, _next: NextFunction): Promise<void> => {
      try {
        const sheet = await this.getSheet()
        ...
        if (!custom) {
          const id = Number.parseInt(request.query.id as string, 10) || -1
          ...
          if (toCaller?.jwt && toCaller?.id) {
            response.cookie('jwt', toCaller.jwt, {
              maxAge: 365 * 24 * 60 * 60,
            })
    ...
    return async (request: Request, response: Response, _next: NextFunction): Promise<void> => {
      try {
        ...
        if (!custom) {
          ...
          const roles: string[] = response?.locals?.jwt?.roles || []
          const list = (await sheet.getList()) || []
          const { toDatabase, toCaller } = await custom(list, request.body, memberId, roles)
