...
import type { VolunteerKnowledge, VolunteerKnowledgeWithoutId } from '@/services/volunteers'
...
const BoxItem: React.FC<Props> = ({ detailedBox, volunteerKnowledge, saveVolunteerKnowledge }): JSX.Element => {
  ...
  useEffect(() => {
    if (!volunteerKnowledge) {
      return
    }

    let providedchoice = 'unknown'

    if (volunteerKnowledge.ok.includes(gameId)) {
      providedchoice = 'ok'
    }
    if (volunteerKnowledge.bof.includes(gameId)) {
      providedchoice = 'bof'
    }
    if (volunteerKnowledge.niet.includes(gameId)) {
      providedchoice = 'niet'
    }
        ...
        {knowledgeChoices.map(({ name, value }) => (
          ...
          >
            <button
              ...
              className={classnames(styles.knowledgeButton, styles[value], choice === value && styles.active)}
            >
              {name}
            </button>
          </li>
