...
const selectTeamsWithVolunteersCandidates = createSelector(
  ...
  selectTeamList,
  (volunteers, teams) => teams.map(({ id, name }: any) => {
...
interface TeamWithCandidatesVolunteerProps {
  ...
  return (
    <li key={id}>
      <span className={styles.volunteerName}>
        ...
        {' '}
        (
        <DaysDisplay dayWishes={dayWishes} />
        )
      </span>
...
interface TeamWithCandidatesProps {
  teamId: number
}
