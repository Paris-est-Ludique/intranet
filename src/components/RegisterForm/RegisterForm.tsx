...
function RegisterForm({ dispatch }: Props): JSX.Element {
  ...
  const festivalFullDate = festivalDateList.find(item => item.id === 1)?.date
  ...
  const intro = (
    <dl className={styles.registerIntro}>
      ...
      <dd>
        <p>
          Un festival en plein air dédié aux
          {' '}
        ...
        <dl className={styles.registerIntro}>
          ...
          <dd>
            <p>
              ...
              {' '}
              <a href="/">fo.parisestludique.fr</a>
              {' '}
              ou en dessous ^^
            </p>
...
export const fetchForRegisterForm = [fetchMiscFestivalDateListIfNeed, fetchMiscMeetingDateListIfNeed]
